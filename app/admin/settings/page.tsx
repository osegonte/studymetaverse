...
const defaultSettings: Setting[] = [
  {
    key: "match_report_price",
    label: "Match Report Price (EUR)",
    value: "29",
    description: "Shown everywhere on the site — homepage, programme detail page, PremiumCTA section.",
    type: "number",
  },
  {
    key: "match_report_currency",
    label: "Currency",
    value: "EUR",
    description: "Currency symbol used for the Match Report price.",
    type: "select",
    options: ["EUR", "USD", "GBP"],
  },
  {
    key: "programmes_count_label",
    label: "Programmes Count Label",
    value: "3,200+",
    description: "Shown in the hero section and trust bar on the landing page.",
    type: "text",
  },
  {
    key: "site_tagline",
    label: "Hero Tagline",
    value: "Find the Best Study Programme in Germany for You",
    description: "Main headline in the landing page hero section.",
    type: "text",
  },
  {
    key: "stripe_mode",
    label: "Stripe Mode",
    value: "test",
    description: "Set to 'live' when you are ready to accept real payments.",
    type: "select",
    options: ["test", "live"],
  },
];
...
export default function SettingsPage() {
  ...
  const handleSave = () => {
    // TODO: await supabase.from('settings').upsert(settings.map(s => ({ key: s.key, value: s.value })))
    setSaved(true);
  ...
  return (
    <div className="max-w-3xl mx-auto">
          ...
          {settings.map((s) => (
            <div key={s.key} className="px-6 py-5 flex flex-col sm:flex-row sm:items-start gap-4">
              <div className="sm:w-64 flex-shrink-0">
                <p className="font-semibold text-gray-800 text-[13.5px]">{s.label}</p>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={handleSave}
          className="px-6 py-2.5 bg-[#1a3c5e] text-white font-bold text-[13.5px] rounded-xl hover:bg-[#14304d] transition-colors shadow-sm"
        >
          Save Settings
        </button>
      </div>
      ...
      <div className="mt-8 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          ...
          <p className="text-gray-400 text-[12.5px] mt-0.5">Add these to your <code className="bg-gray-100 px-1 rounded">.env.local</code> file to connect Supabase and Stripe.</p>
        </div>
